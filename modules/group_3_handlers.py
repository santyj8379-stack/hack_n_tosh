...
def execute(level_name, context):
    ...
    # Target directory path for standalone Group 3 level modules
    group3_dir_path = os.path.join(current_dir, "group_3")
    sub_module_path = os.path.join(group3_dir_path, f"{module_filename}.py")
    
    # ─── ROUTE TO STANDALONE MODULE IF IT EXISTS ─────────────────────────────
    if os.path.exists(sub_module_path) and os.path.exists(os.path.join(group3_dir_path, "__init__.py")):
        try:
            dynamic_path = f"modules.group_3.{module_filename}"
            target_module = importlib.import_module(dynamic_path)
            # Match the execution signature layout
